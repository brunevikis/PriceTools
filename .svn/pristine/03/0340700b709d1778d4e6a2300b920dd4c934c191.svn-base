    ...
    public class Deck : BaseDeck
    {
        ...
        public override Result GetResults()
        {
            ...
            try
            {
                ...
                //ConsoleApp1.Pmo.Pmo pmo = null;

                ConsoleApp1.ReeDat.ReeDat ree = null;
                ConsoleApp1.SistemaDat.SistemaDat sistema = null;

                ConsoleApp1.ParpDat.ParpDat parp = null;
                ConsoleApp1.ConsultaNwd.ConsultaNwd consulta = null;


                var parpPath = files.FirstOrDefault(x => Path.GetFileName(x).ToLower().Equals("parp.dat"));
                if (parpPath != null)
                    parp = (ConsoleApp1.ParpDat.ParpDat)DocumentFactory.Create(parpPath);

                var consultaPath = files.FirstOrDefault(x => Path.GetFileName(x).ToLower().Equals("consulta.nwd"));
                if (consultaPath != null)
                    consulta = (ConsoleApp1.ConsultaNwd.ConsultaNwd)DocumentFactory.Create(consultaPath);

                var reePath = files.FirstOrDefault(x => Path.GetFileName(x).ToLower().Equals("ree.dat"));
                if (reePath != null)
                    ree = (ConsoleApp1.ReeDat.ReeDat)DocumentFactory.Create(reePath);

                var sistemaPath = files.FirstOrDefault(x => Path.GetFileName(x).ToLower().Equals("sistema.dat"));
                if (sistemaPath != null)
                    sistema = (ConsoleApp1.SistemaDat.SistemaDat)DocumentFactory.Create(sistemaPath);
                ...
                if (consulta != null && parp != null)
                {
                    ...
                    foreach (var c_t in ConsoleApp1.ParpDat.MLTLine.campos)
                    {
