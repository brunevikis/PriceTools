...
namespace Compass.ExcelTools.Templates {
    public class Infosheet {
        ...
        public double[] EarmMaxREE
        {
            get
            {
                var vals = new List<double>();
                for (int i = 0; ws.Cells[8 + i, 8].Value != null; i++)
                {

                    var cellValue = ws.Cells[8 + i, 8].Value;
        ...
        public double[] MetaReservatorioREE
        {
            get
            {
                var vals = new List<double>();
                for (int i = 0; ws.Cells[8 + i, 10].Value != null; i++)
                {

                    var cellValue = ws.Cells[8 + i, 10].Value;
                    vals.Add(Convert.ToSingle(cellValue));
        ...
        public double[] EarmREE
        {
            get
            {
                List<double> vals = new List<double>();
                for (int i = 0; ws.Cells[8 + i, 9].Value != null; i++)
                {

                    var cellValue = ws.Cells[8 + i, 9].Value;
                    vals.Add(Convert.ToSingle(cellValue));
